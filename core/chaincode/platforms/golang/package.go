...
import (
	...
	cutil "github.com/idci/core/container/util"
...
func writeChaincodePackage(spec *pb.ChaincodeSpec, tw *tar.Writer) error {
	...
	newRunLine := fmt.Sprintf("RUN go install %s && cp src/hyperledger-bankid/core.yaml $GOPATH/bin && mv $GOPATH/bin/%s $GOPATH/bin/%s", urlLocation, chaincodeGoName, spec.ChaincodeID.Name)
